...
export const api = {
  ...
  toggleContactMemory(contactId: string, enabled: boolean) {
    return requestJson<{ success: boolean }>(`/api/contacts/${contactId}/memory-toggle`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ enabled }),
    });
  ...
  summarizeContactNow(contactId: string) {
    return requestJson<{ success: boolean; memory?: string; last_summary_at?: string | null; reason?: string }>(`/api/contacts/${contactId}/summarize-now`, {
      method: 'POST',
    });
  },
  ...
  // ── AI Voice Notes ──
  getVoiceSettings() {
    return requestJson<{ enabled: boolean; chance: number; maxPerDay: number; bgVolume: number; defaultBgSound: string; telegramVnVoiceId: string; telegramVnVoiceLanguage: string }>('/api/voice-settings');
  },
  updateVoiceSettings(payload: { enabled?: boolean; chance?: number; maxPerDay?: number; bgVolume?: number; defaultBgSound?: string; telegramVnVoiceId?: string; telegramVnVoiceLanguage?: string }) {
